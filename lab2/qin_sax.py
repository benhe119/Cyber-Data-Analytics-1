...
#            numSubsequences = 20
    ...
    def get_results(self, letter_to_rep_result):
        """
        added afterwards

        input the result of to_letter_rep method call and plot the results
        """
        numericalsequence = letter_to_rep_result[0]
        timestamps = letter_to_rep_result[1]

        alphabet_list = []
        time_list = []

        datalength = len(numericalsequence)

        for i in np.arange(0,datalength):
            alphabet_list.append(numericalsequence[i])
            time_list.append(timestamps[i][0])

        return time_list, alphabet_list

    def plot_results(self, x_axis_list, y_axis_list):
        """
        added afterwards

        plots discretized sequence with input sequence
        """
        # print x_axis_list


        plt.figure(1)
        # plt.step(x_axis_list,y_axis_list,where='post')
        plt.plot(x_axis_list, y_axis_list, drawstyle='steps-post')

        t = np.arange(0,self.x.size)

        plt.plot(t,self.x)
        plt.title('Discretized sequence w = ' + str(self.wordSize) + ' h = ' + str(self.alphabetSize))
        plt.xlabel('time stamps')
        plt.ylabel('discrete bin')
        plt.grid()
        plt.show()
